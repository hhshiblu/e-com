...
import { addFriend, sendMessage } from "../../Redux/Action/chat";
import { toast } from "react-toastify";

const socket = io("http://localhost:5000");

const ChatUserDashboard = () => {
  ...
  useEffect(() => {
    socket.emit("add_user", user._id, user);
  }, []);
  ...
  useEffect(() => {
    ...
    if (receverMessage) {
      if (
        ...
        user._id === receverMessage.receverId
      ) {
      ...
      <div className="w-full flex">
        <div className="w-[230px]">
          ...
          <div className="w-full flex flex-col text-slate-600 py-4 h-[400px] pr-3">
            {my_friends?.map((f, i) => (
              <Link
                to={`/account/profile/chat/${f.fdId}`}
                key={i}
...
export default ChatUserDashboard;
